...
- **Link miễn phí** (không cần mua tên miền): `DOMAIN_NAME=boss.<hostname-vps>.hstgr.cloud`
  (hostname xem ở hPanel → VPS, vd `boss.srv1562015.hstgr.cloud`).
...
**3 việc làm 1 lần:**
1. **Để image GHCR ở chế độ Public:** GitHub → repo → **Packages** → `boss-os` → *Package settings* → Visibility = **Public**.
2. **Tạo tài khoản admin** (chọn 1):
   - *Khuyến nghị:* thêm env `BOSS_ADMIN_USER` + `BOSS_ADMIN_PASSWORD` ở ô Environment → mở app **đăng nhập luôn**.
...
```
Mở `http://<ip-vps>:7777` → màn tạo tài khoản admin (xem MÃ THIẾT LẬP trong `docker compose logs boss`).
...
3. Double-click  setup.bat   (chạy hiện cửa sổ)
   hoặc           start-boss.vbs   (chạy ngầm, log ở server\boss.log)
4. Mở http://localhost:7777   ·   Dừng: stop-boss.bat
```
...
|---|---|---|
| `BOSS_HOST` | Địa chỉ nghe. `127.0.0.1`=chỉ máy này; `0.0.0.0`=public | `127.0.0.1` |
| `BOSS_PORT` | Cổng | `7777` |
| `BOSS_REQUIRE_LOGIN` | `1`/`0` ép bật/tắt bắt buộc đăng nhập (mặc định: bật khi bind public) | *(auto)* |
| `BOSS_ADMIN_USER` / `BOSS_ADMIN_PASSWORD` | Tạo sẵn admin lúc deploy (khỏi cần MÃ THIẾT LẬP) | - |
| `BOSS_STATE_DIR` | Nơi ghi state (settings, sessions, loop) | `server/` (Docker: `/data/state`) |
...
cd boss && ./update.sh          # tự pull image + restart (dữ liệu trong volume KHÔNG mất)
```
...
|---|---|
| Sửa code mà không thấy đổi | Đã đổi `.py`? **Khởi động lại server** (Windows: `stop-boss.bat` → `start-boss.vbs`). Đổi giao diện? **Ctrl+Shift+R**. |
| Port 7777 bị giữ, bản mới không lên | Kill tiến trình cũ TRƯỚC (`stop-boss.bat`, hoặc `taskkill /F /PID <pid>`), rồi start lại. |
| Hostinger không pull được image | Để package GHCR = **Public**; đợi GitHub Action build xong (tab Actions). |
| Mở app báo cần MÃ THIẾT LẬP | App terminal (trong container): `cat /data/state/.setup_token`. Trên host: `docker compose logs boss \| grep "SETUP TOKEN"`. Hoặc đặt env `BOSS_ADMIN_PASSWORD` để khỏi cần mã. |
