# Cài đặt Boss OS trên server / VPS
...
> `docker-compose.hostinger.yml` + đặt `DOMAIN_NAME=boss.<hostname-vps>.hstgr.cloud` - xem mục
> **"Link mặc định + HTTPS trên Hostinger"** ở phần HTTPS bên dưới. Compose gốc này chỉ vào được bằng IP:7777.

**3 việc làm 1 lần:**
1. **Để image GHCR ở chế độ Public:** GitHub → repo `boss-os` → **Packages** → `boss-os`
...
2. **Tạo tài khoản admin an toàn** (Claude chạy full quyền nên không để ai cũng tạo được):
   - **Cách A (khuyến nghị):** trong compose của Hostinger, thêm env `BOSS_ADMIN_USER` +
     `BOSS_ADMIN_PASSWORD` → admin tạo sẵn lúc khởi động, mở app ra **đăng nhập luôn**.
...
Mọi ghi chú / vault / settings nằm trong Docker volume (`boss-data`, `claude-auth`) →
...
> Bấm **Manage → sửa .yaml**, đổi thẳng dòng `Host(...)` thành `Host(\`boss.srv1782015.hstgr.cloud\`)`.
...
2. Mở `http://<ip-vps>:7777` → **⚙ Cài đặt → Tên miền riêng** → nhập `boss.tencuaban.com` → **Lưu**.
3. Bấm **Kiểm tra kết nối** → app hiện đúng bản ghi DNS cần tạo (A: `boss.tencuaban.com → <ip-vps>`).
   Trỏ DNS xong, đợi lan.
4. Mở `https://boss.tencuaban.com` → Caddy **tự xin + gia hạn** chứng chỉ ở lần mở đầu, cookie
   Secure tự bật. Xong.
...
```
Quản lý dịch vụ: `journalctl -u boss -f` · `sudo systemctl restart boss`

---

## Cách 3 - Windows (máy cá nhân)

Double-click `setup.bat` (chạy hiện cửa sổ) hoặc `start-boss.vbs` (chạy ngầm).
Dừng bằng `stop-boss.bat`. Mở http://localhost:7777
...
- **Native (systemd)**: `pip install -r requirements.txt` + `systemctl restart boss`.
...
git pull && ./.venv/bin/pip install -r requirements.txt && sudo systemctl restart boss   # Native
```
