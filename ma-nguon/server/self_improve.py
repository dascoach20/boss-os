...
Nâng cấp từ loop ĐƠN (loop_config.json) thành hệ N loop:
  - ĐỊNH NGHĨA loop = file markdown trong vault: <vault>/Boss/loops/<slug>.md
  ...
  - STATE runtime tách riêng, server sở hữu: <vault>/Boss/loop-state.json, key theo slug:
...
class LoopFeature:
    ...
    # ══════════════════════ registry: Boss/loops/<slug>.md ══════════════════════

    def _loops_dir(self, brain: str) -> Path:
        return Path(self.deps.brain_root(brain)) / "Boss" / "loops"
    ...
    def _loop_path(self, brain: str, slug: str) -> Optional[Path]:
        ...
        file PHẢI nằm NGAY TRONG Boss/loops - chặn '../' traversal. None = slug không hợp lệ."""
    ...
    def _state_path(self, brain: str) -> Path:
        return Path(self.deps.brain_root(brain)) / "Boss" / "loop-state.json"
    ...
    # ══════════════════════ state runtime: Boss/loop-state.json ══════════════════════
    ...
    def ensure_migrated(self) -> None:
        """Idempotent: Boss/loops/ chưa có file nào + loop_config.json tồn tại → sinh
        ...
        try:
            ...
            print(f"[loops] đã migrate loop_config.json → Boss/loops/{LEGACY_SLUG}.md",
    ...
    def _log_append(self, brain: str, entry: dict) -> None:
        try:
            d = Path(self.deps.brain_root(brain)) / "Boss" / "loop-log"
            d.mkdir(parents=True, exist_ok=True)
            now = _now_vn()
    ...
    async def _build_prompt(self, loop: dict) -> Tuple[str, str]:
        ...
        if goal == "product":
            base = (
                "MỤC TIÊU: TỰ CẢI THIỆN BOSS hữu dụng hơn với người dùng.\n"
                "Đọc log hội thoại gần đây (Memory/conversations) + các agent/workflow trong Boss/ + ghi chú phản hồi. "
            ...
            if is_write:
                return base + (
                    "Thực hiện 1 cải tiến cụ thể: tạo/cải thiện 1 agent hoặc workflow trong Boss/ (đúng format frontmatter), "
    ...
    def _make_cli(self, loop: dict, cwd: str, sysprompt: Optional[str], for_verify: bool = False) -> Optional[ClaudeCLI]:
        ...
        if loop["tools_profile"] == "code":
            ...
            if self.deps.apply_mcp:
                try:
                    self.deps.apply_mcp(cli, mode="full")   # hub nhận mode qua header X-Boss-Mode
                except TypeError:
