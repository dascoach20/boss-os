// ============================================
// BOSS OS - Console layer (sidebar + router)
// Bọc ngoài cockpit: rail điều hướng + trang quản lý. KHÔNG sửa app.js.
// Graph 3D tự pause khi rời cockpit (qua window.__bossGraph). Alpine cho UI.
...
(function () {
  ...
  const APP_VERSION = "1.0.0";   // fallback hiển thị tức thời; nguồn thật là /version (file VERSION)
  ...
  const RAIL_ITEMS = [
    { id: "home",        icon: ICON.home,        label: "Boss OS" },
    ...
    { id: "overview",    icon: ICON.overview,    label: "Tổng quan", sub: true },
  ...
  // 4 trang tách từ Studio cũ - render container rồi gọi loader trong studio.js (window.BossStudio).
  ...
  function navigateTo(id) {
    ...
    const swap = () => {
      store.active = id;
      // Nút điều khiển cockpit (⚙🔊↻) chỉ hiện ở trang Boss OS, không hiện navbar trang quản lý
  ...
  function renderStudioPage(el, id) {
    ...
    const fn = window.BossStudio && window.BossStudio[id];
  ...
  async function renderFiles(el) {
    ...
    async function load(path) {
      ...
      if (!resp.ok || d.error) {
        const msg = d.error || (resp.status === 404
          ? "Máy chủ Boss OS chưa có chức năng Tệp tin - hãy KHỞI ĐỘNG LẠI server (stop-boss.bat → start-boss.vbs) rồi tải lại trang."
    ...
    async function openFile(rel, it) {
      ...
      if (!resp.ok || d.error) {
        const m = d.error || (resp.status === 404 ? "Server chưa có chức năng Tệp tin - khởi động lại server Boss OS."
  ...
  async function renderSelfImprove(el) {
    ...
    const GNAME = { business: "Kinh doanh", brain: "Bộ não", product: "Cải thiện Boss OS", custom: "Tự định nghĩa" };
    ...
    el.innerHTML = `<div class="cview-section">
      ...
      <div id="lpForm" style="display:none;margin-bottom:14px;padding:14px;border:1px solid rgba(255,255,255,.1);border-radius:10px;background:rgba(255,255,255,.03)">
        ...
        <div class="si-grid">
          ...
          <div class="si-field"><label>Mô tả nhiệm vụ (mỗi vòng Boss OS làm đúng việc này)</label>
          ...
          <div class="dim" style="font-size:12px;color:#6b7894;margin-top:2px">Đề xuất = chỉ đọc + gợi ý. Tự làm (an toàn) = ghi nháp file + đọc MCP, KHÔNG tiền/đơn/đăng bài. Toàn quyền = tự thao tác mọi thứ. · Tinh chỉnh nâng cao (giờ im lặng, trần vòng/ngày, thư mục code): sửa file <code>Boss/loops/&lt;tên&gt;.md</code>.</div>
    ...
    el.querySelector("#lpSave").onclick = async () => {
      ...
      if (!body) { el.querySelector("#lpFormMsg").textContent = "Nhập mô tả nhiệm vụ (Boss OS cần biết mỗi vòng làm gì)"; return; }
    ...
    function loopCard(lp) {
      ...
      div.querySelector(".del").onclick = async () => {
        if (!confirm(`Xoá loop "${lp.name}"? File Boss/loops/${lp.slug}.md sẽ bị xoá.`)) return;
    ...
    async function loadLoops() {
      ...
      box.innerHTML = "";
      if (!(d.loops || []).length) {
        box.innerHTML = `<div class="empty">Chưa có loop nào. Bấm <b>+ Loop mới</b>, hoặc nói với Boss OS trong chat (vd "tạo loop mỗi 2 tiếng đọc 1 source rồi đề xuất").</div>`;
      } else {
  ...
  async function renderLearn(el) {
    ...
    el.innerHTML = `<div class="cview-section">
      ...
      <div class="si-log"><h3 style="font-size:15px;color:#cdd8ee">Boss OS đã tự học gì (commit gần nhất)</h3><div id="lnReview">Đang tải...</div></div>
  ...
  async function renderKanban(el) {
    ...
    el.innerHTML = `<div class="cview-section">
      ...
      <div id="knForm" style="display:none;margin-bottom:14px;padding:14px;border:1px solid rgba(255,255,255,.1);border-radius:10px;background:rgba(255,255,255,.03)">
        ...
        <div class="si-field"><label>Mô tả việc (intent - Boss OS đọc để làm)</label><textarea id="knIntent" placeholder="Mô tả rõ việc cần làm + ghi kết quả nháp vào đâu (vd 05 - Projects)."></textarea></div>
        <div class="si-row" style="gap:14px;flex-wrap:wrap">
    ...
    el.innerHTML = `
      <div class="cview-section">
        ...
        <div class="gcard" style="max-width:560px">
          <div class="gcard-top"><span class="gcard-name">Boss OS</span><span class="gcard-tag" id="ovVerTag">…</span></div>
      ...
      <div class="cview-section">
        ...
        <div class="cgrid">
          ...
          <div class="gcard"><div class="gcard-top"><span class="gcard-name">Workspace</span></div><div class="gcard-meta">${esc(s.workspace_name || "Boss OS")}</div></div>
  ...
  function openApikeyFlow(el, con, isFirst) {
    ...
    m.querySelector("#cGo").onclick = async () => {
      ...
      m.querySelector(".conn-form").innerHTML = '<div class="conn-ok">✓ Đã kết nối: <b>' + esc(r.label || con.name) + '</b> (' + (r.tools || 0) + ' công cụ)'
        + (isFirst ? '<div class="conn-hint">Sang trang Boss OS hỏi thử: "Hôm nay bán được bao nhiêu?"</div>' : "") + '</div>';
      ...
      _connPoll = setInterval(async () => {
        ...
        if (st.state === "qr" && st.qr) {
          ...
          zone.innerHTML = '<div class="conn-ok">✓ Đã đăng nhập: <b>' + esc(st.label || "Zalo") + '</b>'
            + (isFirst ? '<div class="conn-hint">Sang trang Boss OS nhắn thử: "Đọc tin nhắn Zalo mới nhất"</div>' : "") + '</div>';
  ...
  function openFullAck(el, c, con) {
    const text = (con && con.risk) ? con.risk
      : "Mức này cho phép Boss OS thao tác THẬT ra ngoài qua kết nối này: tạo đơn, gửi tin, chạy quảng cáo, publish… Hành động có thể KHÔNG hoàn tác được.";
  ...
  async function renderConnect(el) {
    ...
    if (main.provider === "openai-oauth") {
      warn = `<div class="gcard" style="border:1px solid #2c7a4b;background:rgba(44,122,75,.10);max-width:740px;margin-bottom:14px"><div class="gcard-meta" style="opacity:1">✓ <b>ChatGPT (gói subscription)</b> chạy qua <b>Codex CLI</b> - Boss OS tự đẩy kho Kết nối sang Codex qua hub, nên vẫn dùng được đầy đủ.</div></div>`;
    ...
    el.innerHTML = warn
      ...
      + '<label style="margin-left:8px;cursor:pointer"><input type="checkbox" id="mcpStrict" ' + (d.strict ? "checked" : "") + '> Chỉ dùng kết nối của Boss OS (bỏ MCP sẵn của máy)</label></div>'
  ...
  async function renderChannels(el) {
    ...
    el.innerHTML = `
      <div class="cview-section">
        ...
        <div class="gcard" style="max-width:560px">
          ...
          <input class="js-input" id="tgToken" type="password" placeholder="${tg.token_set ? "Để trống nếu không đổi" : "Dán token 123456:ABC..."}">
    ...
    async function refreshTgStatus() {
      ...
      if (!d.enabled) line = "⚪ Bot CHƯA bật - tích 'Bật bot Telegram' rồi Lưu (test gửi được KHÔNG có nghĩa bot đang nhận tin).";
        ...
        line = `🟢 Bot đang nhận tin - ${n ? n + " chat ID được phép" : "MỌI NGƯỜI nhắn được (chưa giới hạn ID)"} - nhắn cho bot là Boss OS trả lời.`;
      }
  ...
  async function renderAccount(el) {
    ...
    el.innerHTML = `
      <div class="cview-section">
        ...
        <div class="gcard" style="max-width:560px">
          ...
          <input class="js-input" id="acWs" value="${esc(s.workspace_name || "Boss OS")}">
      ...
      <div class="cview-section">
        ...
        <div class="gcard" style="max-width:560px">
          ...
          ${auth.has_password ? `
          <label class="js-lbl">Tài khoản</label><input class="js-input" id="acUser" value="${esc(auth.username || "")}" placeholder="Admin" readonly style="opacity:.7;cursor:not-allowed">
          <label class="js-lbl">Mật khẩu hiện tại</label><input class="js-input" id="acCurPass" type="password" data-pw placeholder="Nhập mật khẩu hiện tại" autocomplete="current-password">
          <label class="js-lbl">Mật khẩu mới</label><input class="js-input" id="acPass" type="password" data-pw placeholder="Nhập mật khẩu mới (tối thiểu 8 ký tự)" autocomplete="new-password">
          <div class="js-actions">
            <button class="gcard-btn" id="acSave">Đổi mật khẩu</button>
            <button class="gcard-btn ghost" id="acLogout">Đăng xuất</button><button class="gcard-btn ghost" id="acDisable">Tắt đăng nhập</button>
          </div>` : `
          <label class="js-lbl">Tài khoản</label><input class="js-input" id="acUser" value="${esc(auth.username || "")}" placeholder="Admin">
          <label class="js-lbl">Mật khẩu</label><input class="js-input" id="acPass" type="password" data-pw placeholder="Đặt mật khẩu (tối thiểu 8 ký tự)" autocomplete="new-password">
          <div class="js-actions">
            <button class="gcard-btn" id="acSave">Đặt mật khẩu</button>
          </div>`}
    ...
    document.getElementById("acWsSave").onclick = async () => {
      ...
      const wn = document.getElementById("workspaceName"); if (wn) wn.textContent = document.getElementById("acWs").value.trim() || "Boss OS";
    };
    ...
    document.getElementById("acSave").onclick = async () => {
      ...
      if (!pass || pass.length < 8) { acStatus.textContent = "⚠ Mật khẩu tối thiểu 8 ký tự."; return; }
      if (auth.has_password) {
        // ĐỔI mật khẩu: bắt buộc nhập đúng mật khẩu hiện tại
        const cur = document.getElementById("acCurPass").value;
        if (!cur) { acStatus.textContent = "⚠ Nhập mật khẩu hiện tại."; return; }
        acStatus.textContent = "Đang đổi...";
        const fd = new FormData(); fd.append("current_password", cur); fd.append("new_password", pass);
        try { const r = await (await fetch("/auth/change-password", { method: "POST", body: fd })).json();
          if (r.ok) { acStatus.textContent = "✅ Đã đổi mật khẩu."; renderAccount(el); }
          else acStatus.textContent = "⚠ " + (r.error || "Lỗi."); }
        catch (e) { acStatus.textContent = "⚠ Lỗi mạng."; }
        return;
      }
      // ĐẶT mật khẩu lần đầu: /auth/setup cấp cookie ngay → tránh tự khoá khi bật auth
  ...
  async function renderSettings(el) {
    ...
    if (window.__bossRefreshExtras) { try { window.__bossRefreshExtras(); } catch (e) {} }  // nạp lại avatar/tên miền
  ...
  document.addEventListener("alpine:init", () => {
    Alpine.store("nav", {
      active: "home",
      items: RAIL_ITEMS,
      settingsOpen: (function () { try { return localStorage.getItem("settingsOpen") === "1"; } catch (e) { return false; } })(),
      get meta() { return VIEW_META[this.active] || VIEW_META.home; },
      toggleSettings() {
        this.settingsOpen = !this.settingsOpen;
        try { localStorage.setItem("settingsOpen", this.settingsOpen ? "1" : "0"); } catch (e) {}
      },
      go(id) {
        const item = RAIL_ITEMS.find(i => i.id === id);
        // "Cài đặt": xổ ra/vào submenu (Tổng quan · Tự học · Cập nhật) + vào trang settings
        if (id === "settings") this.toggleSettings();
  ...
  function boot() {
    ...
    // Thu gọn / mở rộng thanh bên (nhớ trạng thái qua localStorage)
    try {
    ...
    if (railToggle) {
      railToggle.addEventListener("click", function () {
        ...
        try { localStorage.setItem("railCollapsed", collapsed ? "1" : "0"); } catch (e) {}
        // Cột giữa đổi kích thước → resize liên tục trong lúc animate để orb luôn ở GIỮA
        const g = window.__bossGraph;
        if (g && g.resize) {
    ...
    // Footer: hiện "…xem thêm" khi danh sách MCP tràn 1 dòng; bấm để mở rộng
    ...
    if (footMore && mcpListEl && hudFooter) {
      const checkFootMore = function () {
        if (hudFooter.classList.contains("expanded")) return;
        const overflow = mcpListEl.scrollWidth > mcpListEl.clientWidth + 2;
        footMore.style.display = overflow ? "inline" : "none";
      };
      footMore.addEventListener("click", function () {
        const exp = hudFooter.classList.toggle("expanded");
        footMore.textContent = exp ? "thu gọn" : "…xem thêm";
        if (!exp) setTimeout(checkFootMore, 50);
      });
      window.addEventListener("resize", checkFootMore);
      try { new MutationObserver(checkFootMore).observe(mcpListEl, { childList: true, subtree: true }); } catch (e) {}
      setTimeout(checkFootMore, 400);
    }
