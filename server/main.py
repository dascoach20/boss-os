...
from graph_builder import build_graph, _color_for, _top_folder, WIKILINK_RE, _nfc
...
async def _auth_guard(request: Request, call_next):
    ...
    if cfgmod.gate_active():
        ...
        if not public and not cfgmod.valid_session(request.cookies.get("boss_session", "")):
...
def build_system_prompt(brain: str = "brain") -> str:
    ...
    base += (
        ...
        "TUYỆT ĐỐI KHÔNG dùng dấu gạch ngang dài — (em dash) hay – (en dash) trong bất kỳ "
        "phản hồi nào: chat, tiêu đề, danh sách, hay văn bản sinh ra. Thay bằng dấu phẩy, dấu chấm, "
        "dấu hai chấm, hoặc gạch nối ngắn \"-\" tùy ngữ cảnh. Đây là quy tắc không được vi phạm."
    )
    return base
...
def log_conversation(brain: str, user_msg: str, boss_msg: str):
...
# Working directory cho Claude CLI - mặc định là root project Boss OS
...
def _session_cookie(resp, token, request=None):
    ...
    # Chỉ bật khi bạn CHẮC CHẮN HTTPS đầu-cuối: đặt env BOSS_SECURE_COOKIE=1.
    secure = os.getenv("BOSS_SECURE_COOKIE", "").strip().lower() in ("1", "true", "yes", "on")
    ...
    if not secure and request is not None:
        try:
            ...
            pass
    resp.set_cookie("boss_session", token, httponly=True, samesite="lax",
...
async def auth_status(request: Request):
    ...
    has_session = cfgmod.valid_session(request.cookies.get("boss_session", ""))
...
async def auth_change_password(request: Request, current_password: str = Form(...),
                               new_password: str = Form(...)):
    """Đổi mật khẩu - BẮT BUỘC nhập đúng mật khẩu hiện tại. Chỉ gọi được khi đang đăng nhập (middleware chặn)."""
    cfg = cfgmod.read_settings()
    if not cfgmod.auth_enabled(cfg):
        return JSONResponse({"ok": False, "error": "Chưa đặt mật khẩu."}, status_code=400)
    if not cfgmod.verify_password(current_password, cfg):
        await asyncio.sleep(0.5)   # làm chậm dò mật khẩu
        return JSONResponse({"ok": False, "error": "Mật khẩu hiện tại không đúng."}, status_code=403)
    if len(new_password) < 8:
        return JSONResponse({"ok": False, "error": "Mật khẩu mới tối thiểu 8 ký tự."}, status_code=400)
    if new_password == current_password:
        return JSONResponse({"ok": False, "error": "Mật khẩu mới phải khác mật khẩu cũ."}, status_code=400)
    h, salt = cfgmod.hash_password(new_password)
    cfg["auth"]["password_hash"] = h
    cfg["auth"]["salt"] = salt
    cfgmod.write_settings(cfg)
    # cấp session mới → phiên đang mở vẫn hợp lệ
    return _session_cookie(JSONResponse({"ok": True}), cfgmod.new_session(), request)
...
async def auth_logout(request: Request):
    cfgmod.drop_session(request.cookies.get("boss_session", ""))
    resp = JSONResponse({"ok": True})
    resp.delete_cookie("boss_session", path="/")
    return resp
...
async def settings_set(section: str = Form(...), data: str = Form("{}")):
    ...
    if section == "general":
        if "workspace_name" in patch:
            cfg["workspace_name"] = patch["workspace_name"] or "Boss OS"
...
def _node_payload(fpath, roots):
    ...
    try:
        ...
        rel = Path(fpath).name
    stem = _nfc(Path(fpath).stem)   # NFC: khớp wikilink + nhãn không vỡ font (zip macOS = NFD)
    node = {
        "id": stem.lower(),
        "label": stem,
    ...
    try:
        ...
        for m in WIKILINK_RE.finditer(content):
            t = _nfc(m.group(1).strip().split("/")[-1].strip()).lower()
            if t and t != node["id"]:
                targets.append(t)
    except Exception:
        pass
...
async def ws_graph(ws: WebSocket):
    ...
    if cfgmod.gate_active() and not cfgmod.valid_session(ws.cookies.get("boss_session", "")):
...
def _brain_sub(root, new_name: str, old_rel: str) -> Path:
    ...
    Fallback cấu trúc CŨ (<root>/<old_rel>, vd Boss/agents, Memory) nếu mới chưa có →
...
def _ensure_brain_scaffold(root):
    ...
    attachments + Boss/README + memory seed. Dùng cho brain mặc định lẫn brain mới tạo."""
    root = Path(root)
    ...
    jr = root / "Boss" / "README.md"
    if not jr.exists():
        jr.parent.mkdir(parents=True, exist_ok=True)
        jr.write_text(BOSS_README, encoding="utf-8")
    try:
        _brain_memory_dir(str(root))   # memory/ + MEMORY.md seed
    except Exception:
        pass
    try:
        # Năng lực HỆ THỐNG (skill boss-builder/ingest/query/lint + loop tự-cải-tiến): nguồn chuẩn
    ...
    try:
        rebuild_boss_index(str(root))   # chỉ mục tầng vận hành (Boss/index.md)
    except Exception as e:
        print(f"[boss index] {e}", file=__import__('sys').stderr)
...
async def brain_migrate(brain: str = Form("brain")):
    ...
    root = Path(_brain_root(brain))
    moved, skipped = [], []
    for old_rel, new_rel in [("Boss/agents", "agents"), ("Boss/workflows", "workflows"), ("Memory", "memory")]:
...
def _agents_dir(brain):
    return _brain_sub(_brain_root(brain), "agents", "Boss/agents")
def _workflows_dir(brain):
    return _brain_sub(_brain_root(brain), "workflows", "Boss/workflows")
...
async def delete_skill(slug: str = Form(...), brain: str = Form("brain")):
    if system_sync.is_system_skill(slug):
        return JSONResponse({"error": "Skill hệ thống của Boss OS - không xoá được (đi theo "
...
# nghĩa bằng file <vault>/Boss/loops/<slug>.md, state ở <vault>/Boss/loop-state.json,
...
# trong vault (Boss/automations.json) + chèn sẵn "Vòng lặp tự cải thiện" (loop nội bộ).
# ============================================================
def _automations_path(brain):
    return Path(_brain_root(brain)) / "Boss" / "automations.json"
...
def rebuild_boss_index(brain: str) -> dict:
    """Dựng lại Boss/index.md từ file. Chỉ ghi KHI nội dung đổi (chống churn git)."""
    try:
        content = _render_boss_index(_gather_capabilities(brain))
        idx = Path(_brain_root(brain)) / "Boss" / "index.md"
...
def _boss_capability_summary(brain: str) -> str:
    ...
    if not any(c.values()):
        return ""
    parts = ["\n\n# === NĂNG LỰC BOSS HIỆN CÓ (đọc `Boss/index.md` để biết chi tiết + trigger) ==="]
    if c["agents"]:
...
@app.get("/boss/index")
async def boss_index(brain: str = Query("brain")):
    """Dựng lại + trả nội dung Boss/index.md (chỉ mục tầng vận hành)."""
    rebuild_boss_index(brain)
    idx = Path(_brain_root(brain)) / "Boss" / "index.md"
...
async def _start_scheduler():
    ...
    try:
        loop_feature.ensure_migrated()   # loop_config.json cũ → Boss/loops/vong-lap-goc.md (1 lần)
    except Exception as e:
        print(f"[loops migrate] {e}", file=_sys.stderr)
    try:
        if cfgmod.provision_admin_from_env():
            print("[auth] Đã tạo tài khoản admin từ BOSS_ADMIN_PASSWORD (env).", file=_sys.stderr)
        if cfgmod.setup_token_required():
            ...
            print("\n" + "=" * 66 +
                  ...
                  "\n   BOSS_ADMIN_PASSWORD env để tạo sẵn admin, khỏi cần mã.)\n" +
                  "=" * 66 + "\n", file=_sys.stderr)
    except Exception as e:
        ...
        while True:
            try:
                ...
                try:
                    ...
                    print(f"[boss index tick] {type(ie).__name__}: {ie}", file=__import__('sys').stderr)
            except Exception as e:
...
async def config():
    ...
    return {
        "workspace_name": s.get("workspace_name") or os.getenv("WORKSPACE_NAME", "Boss OS"),
...
def _deploy_mode() -> str:
    ...
    if os.path.exists("/.dockerenv") or os.getenv("BOSS_STATE_DIR", "").startswith("/data"):
        return "docker"
    if os.name == "nt":
        return "windows"
    return "native"
    ...
    try:
        ...
        if mode == "windows":
            ...
            with open(bat, "w", encoding="utf-8") as f:
                ...
                f.write('wscript.exe "start-boss.vbs"\r\n')
...
async def domain_set(domain: str = Form("")):
    ...
    if d and not _DOMAIN_RE.match(d):
        return JSONResponse({"ok": False, "error": "Tên miền không hợp lệ (vd: boss.tencuaban.com)"}, status_code=400)
    cfg = cfgmod.read_settings()
...
def _boss_port() -> int:
    try:
        return int(os.getenv("BOSS_PORT", "7777"))
    except ValueError:
        return 7777
...
async def _tg_answer(text, meta=None):
    ...
    sysprompt = build_system_prompt(brain) + channel_context.build_channel_block(
        "telegram", meta, telegram_running=True, port=_boss_port())
...
async def _tg_command(cmd, arg, chat=None):
    ...
    if cmd in ("nhomzalo", "zalogroups", "nhom"):
        ...
        data = r.get("data")
        rows = data.get("groups") if isinstance(data, dict) else (data if isinstance(data, list) else [])
        if not rows:
            return {"reply": "Không thấy nhóm Zalo nào (hoặc tài khoản chưa vào nhóm)."}
        lines = []
        for g in rows[:40]:
...
if __name__ == "__main__":
    ...
    # chạy Claude full quyền trên máy + vault của bạn). Đổi qua BOSS_HOST nếu cần.
    host = os.getenv("BOSS_HOST", "127.0.0.1")
    port = int(os.getenv("BOSS_PORT", "7777"))
