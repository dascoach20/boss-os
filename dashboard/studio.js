// ============================================
// BOSS OS - Studio: Agents / Skills / Workflows
// ============================================
(function () {
  ...
  // cho nút header & dải số liệu .bstat ở đáy graph). Console gọi loader qua window.BossStudio.
  ...
  async function loadAutomations() {
    ...
    panel.innerHTML = `<div class="panel-bar"><h3>Lịch tự động <span class="dim" id="autoRunning"></span></h3><div class="pb-actions"><button class="s-btn-ghost" id="syncAuto">↻ Đồng bộ cloud</button><button class="s-btn" id="newAuto">+ Lịch</button></div></div>`
      + `<div class="auto-hint">Bấm <b>↻ Đồng bộ cloud</b> để Boss OS hỏi Claude (CronList / scheduled tasks) lấy routine THẬT đang chạy trên cloud. Mục ☁ là tự đồng bộ; mục ghi tay vẫn giữ. Các loop 🔁 (trang Tự cải thiện) bật/tắt được ngay tại đây.</div>`
  ...
  function renderSkillList() {
    ...
    list.forEach(s => {
      ...
      const sysBadge = s.system ? ` <span class="sysb" title="Skill hệ thống Boss OS - có ở mọi brain, tự cập nhật theo phiên bản app. Sửa nội dung thì giữ bản của bạn (ngừng tự cập nhật). Không xoá được - chỉ tắt.">hệ thống</span>` : "";
